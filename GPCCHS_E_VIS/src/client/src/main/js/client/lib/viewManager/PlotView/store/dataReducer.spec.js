...
describe('viewManager/PlotView/store/dataReducer', () => {
  test('DATA_REMOVE_ALL_VIEWDATA', () => {
    ...
    expect(plotViewData(state.PlotViewData, action))
      .toEqual({});
  });
  test('HSC_CLOSE_WORKSPACE', () => {
    expect(plotViewData(state.PlotViewData, { type: types.HSC_CLOSE_WORKSPACE }))
      .toEqual({});
  });
  test('WS_VIEW_OPENED', () => {
    const action = {
    ...
    expect(plotViewData(freezeMe({}), action))
      .toEqual({});
  ...
  test('WS_VIEW_CLOSE', () => {
    ...
    expect(plotViewData(freezeMe(frozen), action))
      .toBe(frozen);
    action.payload.viewId = 'myPlot';
    expect(plotViewData(frozen, action))
      .toEqual({});
  });
  test('WS_PAGE_CLOSE', () => {
    ...
    test('ok', () => {
      ...
      const action = {
        ...
        payload: {
          ...
          configurations: { PlotViewConfiguration: state.PlotViewConfiguration },
        },
      };
  ...
  describe('INJECT_DATA_OBSOLETE_EVENT', () => {
    const dataMap = dataMapGenerator(state);
    test('nothing to add', () => {
      const frozen = freezeMe(state.PlotViewData);
      const action = {
        type: types.INJECT_DATA_OBSOLETE_EVENT,
        payload: {
          newViewMap: dataMap.perView,
          dataToInject: {},
        },
    ...
    test('add an obsolete event for view plot1 and entrypoint ATT_BC_REVTCOUNT1 ', () => {
      const frozen = freezeMe(state.PlotViewData);
      const action = {
        type: types.INJECT_DATA_OBSOLETE_EVENT,
        payload: {
