...
export default class VisuWindow extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    ...
    for (let i = 0; i < nextPropsKeys.length; i += 1) {
      if (nextProps[nextPropsKeys[i]] !== this.props[nextPropsKeys[i]]) {
        shouldRender = true;
      }
    }
    return shouldRender;
  ...
  render() {
    ...
    */
    const lowerPercent = lowerPercentOffset;
    const upperPercent = lowerPercent + selectedPercentWidth;
    const currentPercent = lowerPercent + ((upperPercent - lowerPercent) *
      (currentPercentOffset * 0.01));
    ...
    */
    ...
    return (
      <div
        className={
          classnames(
            visuWindowStyles.visuWindow,
            (dragging ? visuWindowStyles.viewportDragging : null),
            (resizing ? visuWindowStyles.viewportResizing : null)
          )
        }
        style={{
      ...
      >
        ...
        */ }
        <span
          cursor="lower"
          className={visuWindowStyles.lower}
          onMouseDown={onMouseDownResize}
          onDoubleClick={toggleTimesetter}
        />
        <span
          cursor="current"
          className={visuWindowStyles.current}
        ...
        <span
          cursor="upper"
          className={visuWindowStyles.upper}
          onMouseDown={onMouseDownResize}
          onDoubleClick={toggleTimesetter}
        />
