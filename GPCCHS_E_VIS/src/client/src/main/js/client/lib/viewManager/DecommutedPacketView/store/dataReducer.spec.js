...
import decommutedPacketViewData from './dataReducer';

describe('viewManager/DecommutedPacketView/store/dataReducer', () => {
  ...
  test('DATA_REMOVE_ALL_VIEWDATA', () => {
    ...
    expect(decommutedPacketViewData(state, action)).toEqual({});
  });
  test('HSC_CLOSE_WORKSPACE', () => {
    ...
    expect(decommutedPacketViewData(state, { type: types.HSC_CLOSE_WORKSPACE })).toEqual({});
  });
  test('WS_VIEW_OPENED', () => {
    const action = { type: types.WS_VIEW_OPENED,
      payload: { view: { type: 'PlotView', uuid: 'myPlot' } } };
    expect(decommutedPacketViewData(freezeMe({}), action)).toEqual({});
    action.payload.view = { type: 'DecommutedPacketView', uuid: 'myDyn' };
    expect(decommutedPacketViewData(freezeMe({}), action)).toEqual({ myDyn: {} });
  });
  ...
  test('WS_PAGE_OPENED', () => {
    const action = { type: types.WS_PAGE_OPENED,
      payload: { views:
          [{ type: 'PlotView', uuid: 'myPlot' }, { type: 'DecommutedPacketView', uuid: 'myDyn' }] } };
    expect(decommutedPacketViewData(freezeMe({}), action)).toEqual({ myDyn: {} });
  });
  test('WS_WORKSPACE_OPENED', () => {
    const action = { type: types.WS_WORKSPACE_OPENED,
      payload: { views:
          [{ type: 'TextView', uuid: 'myText' }, { type: 'DecommutedPacketView', uuid: 'myDyn' }] } };
    expect(decommutedPacketViewData(freezeMe({}), action)).toEqual({ myDyn: {} });
  });
  test('WS_VIEW_CLOSE', () => {
    ...
    expect(decommutedPacketViewData(freezeMe(frozen), action)).toBe(frozen);
    action.payload.viewId = 'myDyn';
    expect(decommutedPacketViewData(frozen, action)).toEqual({});
  });
  test('WS_PAGE_CLOSE', () => {
    const action = { type: types.WS_PAGE_CLOSE, payload: { viewIds: ['myDyn', 'myText'] } };
    expect(decommutedPacketViewData(freezeMe({ myDyn: {}, myOtherDyn: {} }), action))
      .toEqual({ myOtherDyn: {} });
  });
  test('Unknown action', () => {
  ...
  describe('INJECT_DATA_LAST', () => {
    ...
    test('valid viewData with empty state', () => {
      ...
      expect(decommutedPacketViewData(freezeMe({ decommutedPacket: {} }), action)).toEqual({
        decommutedPacket: {
          index: 10,
          value: {
