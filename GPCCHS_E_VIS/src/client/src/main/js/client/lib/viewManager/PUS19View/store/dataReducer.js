...
function pus19DataReducer(state = {}, action) {
  switch (action.type) {
    case INJECT_PUS_DATA: {
      ...
      const data = _.getOr(null, ['payload', 'data', VM_VIEW_PUS19], action);
      ...
      updatedState = injectTabularData(updatedState, 'onBoardStorages',
        _.getOr(null, ['pus019PacketStore'], data)
        .map(store => ({
          ..._.omit(['pus019Packet'], store),
      ...
      updatedState = injectTabularData(updatedState, 'storageDef',
        _.getOr([], ['pus019PacketStore'], data)
          .reduce((acc, store) => [...acc, ...store.pus019Packet], [])
          .map(packet => ({
            ...packet,
...
export default createScopedDataReducer(pus19DataReducer, {}, VM_VIEW_PUS19);

export const getPUS19ViewData = state => state[`${VM_VIEW_PUS19}Data`];
export const getData = (state, { viewId }) => state[`${VM_VIEW_PUS19}Data`][viewId];
export const getConfiguration = (state, { viewId }) => state[`${VM_VIEW_PUS19}Configuration`][viewId];
