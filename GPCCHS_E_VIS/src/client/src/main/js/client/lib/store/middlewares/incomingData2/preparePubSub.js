...
import { isDataIdInDatamapLast, isTimestampInLastInterval } from '../../../dataManager/mapSelector';
      ...
      if (!payloadsJson[tbdId] || !payloadsJson[tbdId][timestamp.ms]) {
        ...
        payloadsJson = updateFinalPayload(state,
          { tbdId,
            timestamp: timestamp.ms,
            decodedPayload,
            isInIntervals: isTimestampInLastInterval,
            filters },
          payloadsJson);
      }
    }
 ...
 * @param {Object}
            ...
            {number} timestamp: timestamp to check
            {Object} decodedPayload: the decoded payload
            {function} isInIntervals: check if data is in intervals
            {Array} filters
...
export const updateFinalPayload = (
  ...
  if (isInIntervals(state, { tbdId, timestamp })) {
    ...
    if (applyFilters(decodedPayload, filters)) {
      ...
      if (addRecord) {
        addRecord(tbdId, { timestamp: timestamp.ms, payload: decodedPayload });
      }
      // Add data to final object with the current tbdId
      if (!updatedPayloads[tbdId]) {
        updatedPayloads[tbdId] = {};
      }
      updatedPayloads[tbdId][timestamp.ms] = decodedPayload;
    }
  }
