...
describe('protobuf/isis/pusGroundModel/Pus011Model', () => {
  ...
  test('decode', () => {
    ...
    expect(decoded).toMatchObject({
      ...
      scheduleStatus: { type: 'uoctet', value: stub.scheduleStatus },
      ...
      lastUpdateModeFreeSpace: { type: 'uoctet', value: stub.lastUpdateModeFreeSpace },
      lastUpdateModeNoFreeCommands: { type: 'uoctet', value: stub.lastUpdateModeNoFreeCommands },
      lastUpdateModeScheduleStatus: { type: 'uoctet', value: stub.lastUpdateModeScheduleStatus },
    ...
    for (let i = 0; i < stub.pus011Apid.length; i += 1) {
      expect(decoded.pus011Apid[i]).toMatchObject({
        status: { type: 'uoctet', value: stub.pus011Apid[i].status },
        pusElement: {
          lastUpdateMode: { type: 'uoctet', value: stub.pus011Apid[i].pusElement.lastUpdateMode },
