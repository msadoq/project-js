...
// VERSION : 1.1.2 : DM : #5828 : 23/03/2017 : Fix explorer opening with fresh state (no tabId is
//  stored)
// VERSION : 1.1.2 : DM : #5828 : 24/03/2017 : Fix explorer opening with fresh state (no tabId is
//  stored)
...
const panels = (state = initialState, action) => {
  switch (action.type) {
    ...
    case types.WS_PAGE_PANELS_LOAD_IN_SEARCH:
      return { ...state, searchViewsIds: action.payload.viewsIds };
