...
describe('intervals/getIncludesTimestamp', () => {
  ...
  describe('multi', () => {
    test('merged', () => {
      ...
      expect(getIncludesTimestamp([[0, 1], [2, 6]], 5)).toMatchObject({ isInInterval: true, interval: [2, 6] });
    });
    test('unmerged', () => {
      expect(getIncludesTimestamp([[0, 5], [3, 10]], 2)).toMatchObject({ isInInterval: true, interval: [0, 5] });
      expect(getIncludesTimestamp([[0, 5], [3, 10]], 7)).toMatchObject({ isInInterval: true, interval: [3, 10] });
    });
  });
});
