...
describe('patchReducer', () => {
  test('returns same state if "regular action"', () => {
    ...
    expect(patchReducer(state, { [REDUX_SYNCHRONIZATION_PATCH_KEY]: {} })).toBe(state);
    expect(patchReducer(state, { [REDUX_SYNCHRONIZATION_PATCH_KEY]: true })).toBe(state);
    expect(patchReducer(state, { [REDUX_SYNCHRONIZATION_PATCH_KEY]: 'string' })).toBe(state);
  });
  test('applies patch if "patch action"', () => {
    ...
    const s = expect(patchReducer(original, { [REDUX_SYNCHRONIZATION_PATCH_KEY]: patch }));
    s.not.toBe(original);
