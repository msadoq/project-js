...
describe('viewManager/DecommutedPacketView/store/viewDataUpdate', () => {
  describe('Update', () => {
  ...
  describe('selectDataPerView', () => {
    ...
    test('state undefined', () => {
      const data =
        selectDataPerView(viewDataMap.decommutedPacket, expectedIntervals, payload, {});
    ...
    test('visuWindow duration too long', () => {
      ...
      const data = selectDataPerView(viewDataMap.decommutedPacket, intervals, payload, oldState);
      expect(data).toEqual({ });
    });
  });
});
