...
const loki = {};
const lokiManager = {
  addRecord: (tbdId, data) => {
    if (!loki[tbdId]) {
      loki[tbdId] = {};
    }
    loki[tbdId][data.timestamp] = data.payload;
  },
};
...
describe('store:middlewares:preparePubSub', () => {
  ...
  const store = mockStore({
    ...
    knownRanges: {
      ...
      'Reporting.STAT_SU_PID<ReportingParameter>:extracted.>.1': {
        flatDataId: 'Reporting.STAT_SU_PID<ReportingParameter>:1:1',
        filters: [],
        intervals: [[10, 20], [1420106790800, 1420106790850]],
      },
  ...
  const decodedPayload = { myValue1: 1, myValue2: 2 };
  const isInIntervalsStore = (state, { tbdId, timestamp }) => {
  ...
  describe('updateFinalPayload', () => {
    test('', () => {
      const finalPayloads = updateFinalPayload(store.getState(),
        ...
        {});
      expect(finalPayloads).toEqual({ tbdId1: { 10: decodedPayload } });
    });
  });
  describe('updateFinalPayload', () => {
