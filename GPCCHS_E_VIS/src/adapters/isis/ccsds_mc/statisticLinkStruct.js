...
module.exports = {
  encode: (data) => {
    const statisticLinkStruct = new ByteBuffer(null, ByteBuffer.LITTLE_ENDIAN);
    statisticLinkStruct.writeInt32(data.collectionInterval, COLLECTIONINTERVAL_OFFSET);
    statisticLinkStruct.writeInt32(data.reportingInterval, REPORTINGINTERVAL_OFFSET);
    statisticLinkStruct.writeInt32(data.samplingInterval, SAMPLINGINTERVAL_OFFSET);
    statisticLinkStruct.writeByte(data.reportingEnabled, REPORTINGENABLED_OFFSET);
    statisticLinkStruct.writeUint64(data.startTime, STARTTIME_OFFSET);
    return { value: statisticLinkStruct.buffer };
  },
  decode: data => ({
    type: 'raw',
    value: data.value,
