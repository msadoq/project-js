...
import { askItemMetadata, askCatalogItemReportingItemPackets } from '../../actions/catalogs';
...
const onNeededMetadata = ({ dispatch, getState }) => next => (action) => {
  ...
  const _askEntryPointRelatedMetadata = (entryPoint) => {
    ...
    dispatch(askCatalogItemReportingItemPackets(domainId, sessionId, catalog, catalogItem));
  };
