...
describe('protobuf/isis/pusModelEditorMessages/PusMmePacket', () => {
  const builder = new ProtoBuf.Root()
    .loadSync(`${__dirname}/PusMmePacket.proto`, { keepCase: true })
    .lookup('pusModelEditorMessages.protobuf.PusMmePacket');
  let buffer;
  test('encode', () => {
  ...
  test('decode', () => {
    ...
    for (let i = 0; i < stub.pusMmePacketParameter.length; i += 1) {
      expect(decoded.pusMmePacketParameter[i]).toMatchObject({
        ...
        lastUpdateModeParameterId: { type: 'uinteger', value: stub.pusMmePacketParameter[i].lastUpdateModeParameterId },
        lastUpdateTimeParameterId: { type: 'string', value: stub.pusMmePacketParameter[i].lastUpdateTimeParameterId },
