...
import { VM_VIEW_PUS18 } from 'viewManager/constants';
...
export default class PUS18Editor extends Component {
  static propTypes = {
    // own props
    ...
    configuration: PropTypes.shape({
      ...
      tables: PropTypes.shape({
        onBoardCtrlProcedures: PropTypes.shape({
          cols: PropTypes.arrayOf(TableConfigurationColumnType).isRequired,
        }).isRequired,
        procedureParameters: PropTypes.shape({
          cols: PropTypes.arrayOf(TableConfigurationColumnType).isRequired,
        }).isRequired,
      }).isRequired,
    }).isRequired,
   ...
   */
  state = {
    PUS18EditorForm: WithForm(DefaultPusDataContainer),
  };
  ...
  render() {
    ...
    const { PUS18EditorForm } = this.state;
    return (
      <ErrorBoundary>
