...
describe('protobuf/lpisis/tcHistory/TCFile', () => {
  const fixture = stubData.getTCFile();
  let buffer;
  it('encode', () => {
    buffer = protobuf.encode('lpisis.tcHistory.TCFile', fixture);
    buffer.constructor.should.equal(Buffer);
  });
  it('decode', () => {
    const json = protobuf.decode('lpisis.tcHistory.TCFile', buffer);
    ...
    for (let i = 0; i < fixture.tc13.length; i += 1) {
      json.tc13[i].should.be.an('object').that.have.properties({
        ...
        sequenceCount: (typeof fixture.tc13[i].sequenceCount === 'undefined') 
          ? null 
          : { type: 'ulong', value: fixture.tc13[i].sequenceCount },
      });
