...
// VERSION : 1.1.2 : DM : #6785 : 13/06/2017 : read link defined with absolute path, FMD path or
//  OID
        ...
        <Row>
          {show &&
            ...
            (<div key={'div'.concat(ikey)}>
              <Col xs={12} key={link.name.concat(ikey)}>
                <Button onClick={() => this.onClick(ikey)} >{link.name}</Button>
                <Glyphicon
                  glyph="remove"
